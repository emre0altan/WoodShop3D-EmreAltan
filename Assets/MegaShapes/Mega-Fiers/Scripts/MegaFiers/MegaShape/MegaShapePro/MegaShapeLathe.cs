...
public class MegaShapeLathe : MonoBehaviour
{
	...
	MegaPolyShape MakePolyShape(MegaShape shape, int steps)
	{
		...
		lp1 = shape.splines[curve].knots[0].p + axis;
		p1 = Vector3.zero;
		...
		for ( int i = 0; i < kcount; i++ )
		{
			...
			for ( int j = first; j < steps; j++ )
			{
				...
				p1 = shape.splines[curve].knots[k1].InterpolateCS(alpha, shape.splines[curve].knots[k2]);	//]  .Interpolate(i, alpha, true);

				p1 += axis;
				pshape.points.Add(p1);

				//pshape.flags.Add(0);	// All smooth for now
				length += Vector3.Distance(p1, lp1);
		...
		p1 = shape.splines[curve].knots[k2].p;
	...
	Matrix4x4 axismat, iaxis, rotmat, uvmat, tm;
	Quaternion q;
	int vertLevels, vindex;
	float totlen, alphasub, ang, cumlen, halpha, adj, tang,c,s;
	Vector2 uv,uv1;
	Vector3 p, scl, lsp, nn;
	Vector3[] n;
	...
	public void BuildMesh(Mesh mesh)
	{
		...
		vertLevels = segments + 1;

		axismat.SetTRS(axis, q, Vector3.one);
		iaxis = axismat.inverse;

		rotmat = Matrix4x4.identity;

		totlen = pshape.length[pshape.length.Count - 1];

		uv = Vector2.zero;
		uv1 = uv;

		uvmat = Matrix4x4.identity;
		...
		for ( int level = 0; level < vertLevels; level++ )
		{
			...
			ang = (uv.y * degrees) + startang;	//360.0f;
			rotmat = Matrix4x4.identity;
			...
			for ( int v = 0; v < pshape.points.Count; v++ )
			{
				...
				halpha = alphasub - ((lsp.z - min.z) / limits.z);
				...
				if ( usescalecrv )
				{
					...
					adj = scaleamthgt.Evaluate(halpha) * globalscale;
				...
				if ( twist != 0.0f )
				{
					tang = ((twist * halpha * twistcrv.Evaluate(halpha)) + ang) * Mathf.Deg2Rad;
					c  = Mathf.Cos(tang);
					s = Mathf.Sin(tang);
		...
		if ( degrees == 360.0f )
		{
			...
			for ( int v = 0; v < pshape.points.Count; v++ )
			{
				nn= n[v] + n[n.Length - pshape.points.Count + v];
