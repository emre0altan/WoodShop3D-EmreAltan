...
public class MegaShapeEditor : Editor
{
...
#if UNITY_2017_2 || UNITY_2017_3 || UNITY_2017_4 || UNITY_2018 || UNITY_2019
 ...
 // call this function in OnGUI before calling TextField (only needs to be called once)
			...
			if ( spos != bounds.min )
			{
				if ( shape.usesnap )
				{
					if ( shape.snap.x != 0.0f )
						spos.x = Mathf.Round(spos.x / shape.snap.x) * shape.snap.x;

					if ( shape.snap.y != 0.0f )
						spos.y = Mathf.Round(spos.y / shape.snap.y) * shape.snap.y;

					if ( shape.snap.z != 0.0f )
						spos.z = Mathf.Round(spos.z / shape.snap.z) * shape.snap.z;
				}
	...
	Vector3 PosHandlesSnap(MegaShape shape, Vector3 pos, Quaternion q)
	{
		...
		{
			if ( shape.snap.x != 0.0f )
				pos.x = Mathf.Round(pos.x / shape.snap.x) * shape.snap.x;

			if ( shape.snap.y != 0.0f )
				pos.y = Mathf.Round(pos.y / shape.snap.y) * shape.snap.y;

			if ( shape.snap.z != 0.0f )
				pos.z = Mathf.Round(pos.z / shape.snap.z) * shape.snap.z;
		}
